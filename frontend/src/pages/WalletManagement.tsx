...
const WalletManagement: React.FC = () => {
  ...
  const fetchWallets = async () => {
    setLoading(true);
    try {
      const response = await axios.get('/api/plugins/wallet-monitor/wallets');
  ...
  const handleDeleteWallet = async (walletId: number) => {
    try {
      await axios.delete(`/api/plugins/wallet-monitor/wallets/${walletId}`);
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (editingWallet) {
        // 编辑钱包
        await axios.put(`/api/plugins/wallet-monitor/wallets/${editingWallet.id}`, values);
        message.success('钱包更新成功');
      } else {
        // 添加钱包
        await axios.post('/api/plugins/wallet-monitor/wallets', values);
