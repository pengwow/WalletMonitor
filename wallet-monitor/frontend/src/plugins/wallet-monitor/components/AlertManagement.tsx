...
const AlertManagement: React.FC = () => {
  ...
  const fetchAlerts = async () => {
    ...
    try {
      ...
      const response = await axios.get('/api/plugins/wallet-monitor/alerts', { params });
  ...
  const fetchAlertRules = async () => {
    ...
    try {
      const response = await axios.get('/api/plugins/wallet-monitor/alerts/rules');
  ...
  const handleResolveAlert = async (alertId: number) => {
    try {
      await axios.post(`/api/plugins/wallet-monitor/alerts/resolve/${alertId}`);
  ...
  const handleTestAlert = async (walletAddress: string, chain: string, alertType: string) => {
    try {
      await axios.post('/api/plugins/wallet-monitor/alerts/test', {
  ...
  const handleDeleteRule = async (ruleId: number) => {
    try {
      await axios.delete(`/api/plugins/wallet-monitor/alerts/rules/${ruleId}`);
  ...
  const handleSubmitRule = async () => {
    try {
      ...
      if (editingRule) {
        // 编辑规则
        await axios.put(`/api/plugins/wallet-monitor/alerts/rules/${editingRule.id}`, values);
        message.success('告警规则更新成功');
      } else {
        // 添加规则
        await axios.post('/api/plugins/wallet-monitor/alerts/rules', values);
